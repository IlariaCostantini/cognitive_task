...
# load the "reward structure" - I am actually loading an excel spreadsheet which contains the outcome they'll receive. 
# If you change location, chenge the file_path as well 
...
def load_conditions():
    ...
    if sys.platform.startswith('macOS'):  # are these the right info?
        file_path = {'condition': r'..O:\bandit_task\bandit_task\COCOs BabyBandit_IC\condition.xlsx'}  #CHANGE FILE_PATH IF YOU CHANGE COMPUTER/LOCATION
...
def init_elements_in_window():
    ...
    rectangle_right = psychopy.visual.Rect(win=window, units="pix", width=toy_size[0] + 10, height=toy_size[1] + 10,  #change to white? 
                                           lineColor='grey', colorSpace='rgb', pos=pos_right)
    rectangle_left = psychopy.visual.Rect(win=window, units="pix", width=toy_size[0] + 10, height=toy_size[1] + 10,  #change to white? 
                                          lineColor='grey', colorSpace='rgb', pos=pos_left)
    ...
    right_highlight = psychopy.visual.Rect(win=window, pos=pos_right, width=250, height=250, color=(-0.5, -0.5, -0.5), #changed to dark grey! 
                                           units='pix')
    left_highlight = psychopy.visual.Rect(win=window, pos=pos_left, width=250, height=250, color=(-0.5, -0.5, -0.5),  #changed to dark grey! 
                                          units='pix')
...
def init_window():
    window = psychopy.visual.Window(
       ...
       # winType='pygame',
        size=screen_size,
        fullscr=False,    # CHANGE TO True when you run the actual experiment in order to have FULL sCREEN SIZE!!! --------IMPORTANT!!! -----------
        color=[0, 0, 0])
    return window
...
def show_dialog_and_get_info():
    ...
    available_genders = ['m', 'f', 'o'] # I leave here all the genders in case we want to do something with the fathers.
    available_conditions = [1]  #Now just condition 1 is available (soothing the baby)
    available_ages = list(range(13, 55)) #changed from 18 to 51 to 13-55 so that we don't exclude pregnant women at various ages (really young or tardive) 
...
def show_messages_and_wait(window, cpt):  #extracted from the brackets the components of the time horizon line - put them back if you want it:  , rectangle_time, rectangle_time_passing
...
def position_arms(window, is_even, toy_bear, toy_duck, pos_left, pos_right):  #switch the position of the toys every 10 trials? 
...
def experiment_trial(is_reward, window, right_highlight, left_highlight, toy_bear, toy_duck,
                     happy_face, neutral_face, sad_face, cpt_iteration):  #remoted time horizon from the brackets, if you want it put it back :  , rectangle_time, rectangle_time_passing
...
if __name__ == "__main__":
    ...
    while True:
        ...
        experiment_trial(is_reward, window, right_highlight, left_highlight, toy_bear, toy_duck,
                         happy_face, neutral_face, sad_face, cpt_iteration)    #removed:   , rectangle_time, rectangle_time_passing   ## put it back in the line below if you want the time horizon:  # compute_progress(rectangle_time, rectangle_time_passing, cpt_iteration)  #don't need this line because we "silenced" the function for compute progress 
        cpt_iteration += 1
