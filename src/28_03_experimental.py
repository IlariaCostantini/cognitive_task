...
dataFile_reward = pd.read_excel(r'C:\Users\ic18563\OneDrive - University of Bristol\python different\python start\Experimental - Copy\sensitivity_reward_condition_exc.xlsx') #, skiprows=1, nrows= 100, use_cols = "A:B")  TAKE JUST THE 100 rows with data
dataFile_punishment = pd.read_excel(r'C:\Users\ic18563\OneDrive - University of Bristol\python different\python start\Experimental - Copy\sensitivity_punishment_condition_exc.xlsx') #, skiprows=1, nrows= 100, use_cols = "A:B")  TAKE JUST THE 100 rows with data
...
df = pd.ExcelFile(r'C:\Users\ic18563\OneDrive - University of Bristol\python different\python start\Experimental - Copy\sensitivity_reward_condition_exc.xlsx').parse(
    'sensitivity_reward')
...
df = pd.ExcelFile(r'C:\Users\ic18563\OneDrive - University of Bristol\python different\python start\Experimental - Copy\sensitivity_punishment_condition_exc.xlsx').parse(
...
INSTRUCTIONS_PUNISHMENT_EXP = """
    Over the fixation point a happy baby face will be presented, 
    your task is to keep the baby happy with one of the 2 toys available. 
    One of the two is more successful and it will keep the baby smiling! 
    The other toy may stop the baby from being happy (neutral face) or may even distress the baby!
...
def show_messages_and_wait(window, cpt):
    messages = {'welcome': WELCOME,
                'break' : BREAK,
                'thanks': THANKS}
    this_message = {0: 'welcome', 50: 'break', 100: 'thanks'}  #return welcome to 1
    try: 
...
def experiment_trial(is_reward, is_even, position_arms, window, right_highlight, left_highlight, toy_bear, toy_duck, happy_face, neutral_face, sad_face):
 ...
 #   pressed_left = None  #sometimes is actually giving me back None, for example I press left and nothing happen 
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if cpt_iteration == 100:  #to have N you need to put n+1
           break 
    ...
    with open(filename, 'a') as file:
            ...
            for i in range(0, len(pressed_lefts)):  #change back to reaction_times
